...
def smooth_ca_time_series4(diff):
    # Adjusted weights for increased smoothing
    temp = (diff[-1] * 0.5 +  # Increased weight for the last element
            diff[-2] * 0.25 +  # Decreased weight for the second-last element
            diff[-3] * 0.15 +  # Decreased weight for the third-last element
            diff[-4] * 0.075 +  # Increased weight for the fourth-last element
            diff[-5] * 0.075)  # Increased weight for the fifth-last element
...
def smooth_ca_time_series_renan(data, window_size, sigma):
    """
    Apply a Gaussian-weighted average filter to the data.
    ...
    Returns:
    float: The mean of the filtered data.
    """
